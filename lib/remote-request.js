...
RemoteRequest.prototype.start = function() {
  ...
  if (this.cache.headers.content) {
    var etag = this.cache.headers.content.parsed.etag;
    if (etag && (opts.method === 'GET' || opts.method === 'HEAD')) {
      debug('using etag %s for %s %s', etag, opts.method, opts.uriObj.href);
...
RemoteRequest.prototype._onCacheDirCreated = function(err) {
  ...
  var cacheHeaders = {
    parsed: this.response.headers,
    raw: this.response.rawHeaders
  };
  fs.writeFile(
    this.cache.headers.path,
    JSON.stringify(cacheHeaders, null, 2) + '\n',
